...
- [ ] 21. Capa UI (renderer) — construida sobre el núcleo ya probado
  ...
  - [ ]* 21.2 Implementar la sección "Activos" y el reordenamiento de Priority_Order
    ...
    - **Adición de scope, hecha en esta misma rama (`ui`) antes del componente de UI propiamente dicho** (ver `Context/04-historial-decisiones.md`): el mockup de referencia (Claude Design, "2b - Compacto con flechas") exige un preview de colisiones de solo lectura ANTES de aplicar, capacidad que no existía. Se documenta acá (sección/rama de origen de la necesidad) y NO como tareas retroactivas bajo las Secciones 18/20 (ya cerradas y mergeadas a `main` en otras ramas) — ver la regla de `CONTRIBUTING.md`. Subtareas:
      - [x] 21.2a Implementar `previewActiveSet` (`MergeOrchestrator`, `merge-orchestrator.ts`) y `preview` (`MergeEngine`, `merge-engine.ts`) — dominio de la Sección 18
        - Cálculo de SOLO LECTURA del `MergeReport` de un Active_Set candidato: `MergeEngine.preview` lista cada VPK (`VpkTool.list`, sin extraer ni empaquetar) y delega en `resolveMerge` (núcleo puro ya validado por la Property 11); `MergeOrchestrator.previewActiveSet` reutiliza `#resolveOrderedAddons` y NO pasa por `ProcessGuard`, `ElevationService` ni el guard de escrituras (`operationInFlight` de `ipc-handlers.ts`): no escribe nada en disco. Un `VpkTool.list()` fallido para un addon lo excluye BEST-EFFORT del cálculo (`unavailable`) en vez de abortar todo, a diferencia de `merge()` (DECISIÓN 7 de `merge-engine.ts`/`merge-orchestrator.ts`)
        - _Requirements: adición de scope fuera de 1-9 (no un Requirement formal), ver `Context/04-historial-decisiones.md`_
      - [x] 21.2b Agregar el canal `activeSet:preview` — dominio de la Sección 20
        - Canal + `L4d2Api.previewActiveSet` en el contrato (`ipc-contract.ts`), handler `ipcMain.handle` que delega en `MergeOrchestrator.previewActiveSet` SIN pasar por `guardedWrite` (`ipc-handlers.ts`, es de solo lectura, no compite con apply/add/remove), e implementación en el preload
        - _Requirements: adición de scope fuera de 1-9, ver `Context/04-historial-decisiones.md`_
      - [x]* 21.2c Escribir property test de equivalencia preview↔apply
        - **Feature: l4d2-versus-addon-manager, Property 16: Preview y apply reportan las mismas colisiones para el mismo Active_Set**
        - **Validates: comportamiento de `previewActiveSet` frente a `applyActiveSet`, Requirements 6.7, 7.1, 7.2**
        - fast-check con `MergeEngine` REAL conectado a un `VpkTool` fake compartido entre ambos caminos (NO el `FakeMergeEngine` de `orchestrator-doubles.ts`, que volvería trivial la comparación), mínimo 100 iteraciones; el `MergeReport` de `previewActiveSet` es idéntico al de `applyActiveSet` para el mismo Active_Set
      - [x]* 21.2d Escribir unit tests de `previewActiveSet`/`preview`
        - Verifica: addon ausente del escaneo -> `kind: "addon-missing"`; `VpkTool.list()` fallido para un addon -> excluido en `unavailable`, el resto se calcula igual; Active_Set vacío -> preview vacío; CERO invocaciones a `ProcessGuard`/`ElevationService`/`BackupManager`/`GameInfoEditor`/`MergeFileSystem`/`CollisionResolver` (dobles que lanzan si se invocan)
        - _Requirements: adición de scope fuera de 1-9, ver `Context/04-historial-decisiones.md`_
...
- **Property-based testing con fast-check**: las 16 Correctness Properties del diseño se cubren en tareas 2.2, 2.4, 2.6 (Props 8, 9, 10), 5.2, 5.4 (Props 1, 2), 6.2, 6.3 (Props 3, 4), 7.2 (Prop 5), 8.2 (Prop 6), 10.3 (Prop 11), 12.2 (Prop 7), 13.2 (Prop 12), 15.2 (Prop 14), 17.3 (Prop 15), 18.3 (Prop 13) y 21.2c (Prop 16). Cada propiedad es un test independiente, con mínimo 100 iteraciones y etiqueta `Feature: l4d2-versus-addon-manager, Property N: ...`. La Property 16 es una adición posterior fuera del scope original, documentada bajo la Sección 21.2 (adición de scope hecha en la rama `ui`, ver `Context/04-historial-decisiones.md` y la regla de `CONTRIBUTING.md`), no parte de las 15 formalizadas al cerrar el diseño inicial.
