...
### [2026-09-09] Sección 18 (MergeOrchestrator): tercer estado de OperationResult, P-15 y P-14 resueltos, FS propio y decisiones inferidas de add/remove

**Qué:** Se implementó la Sección 18 (MergeOrchestrator, Requisitos 4, 5, 6, 8, 9.2) en la rama `merge-orchestrator` desde `main` actualizada. Nuevos: `src/main/domain/merge-orchestrator.ts` (18.1 + 18.2), `test/merge-orchestrator.property.test.ts` (18.3, Property 13), `test/merge-orchestrator.test.ts` (18.4) y el helper compartido `test/helpers/orchestrator-doubles.ts`. Además se resolvieron los pendientes P-15 y P-14 y se rediseñó `OperationResult`.

**Parte A — P-15 RESUELTO (`ensureCanWrite` recibe `operationType`):** se extendió `ElevationService.ensureCanWrite(gameRoot, entries, operationType: PendingOperation["type"])` para construir la `PendingOperation` con el tipo REAL de la operación (`applyActiveSet`/`addAddon`/`removeAddon`) en vez de hardcodear `"applyActiveSet"`. Se sumó el parámetro con el mismo criterio de la DECISIÓN 1 de `elevation-service.ts` (divergencia consciente de design.md). `handleWriteFailure` NO cambió (ya recibía el `pending` correcto desde afuera). Se actualizaron los 6 llamados de `ensureCanWrite` en `test/elevation-service.test.ts` (todos con `"applyActiveSet"`); el property test `test/elevation-service.property.test.ts` NO se tocó (solo ejercita `handleWriteFailure`).

**Parte B — `OperationResult` rediseñado a TRES estados (DIVERGENCIA CONSCIENTE de design.md):** se pasó de la unión de dos ramas por `ok` (fijada en la Tarea 1) a una unión de tres ramas discriminada por `status: "success" | "failure" | "elevating"`. Motivo: el `elevated-handoff` del `ElevationOutcome` es un desenlace que NO es éxito ni fallo (la instancia se cede a una instancia elevada que reportará por su cuenta, 18.2) y no tenía dónde reportarse en una unión de solo éxito/fallo. Se eligió un discriminante de tres literales (estilo `ElevationOutcome.kind`/`PathDetectionResult.kind`) en vez de reusar `ok`: un booleano no representa tres estados. `success` lleva `report?`/`installedManifest?`; `failure` lleva `error`/`addonId?`; `elevating` no lleva datos. Como HOY no había ningún consumidor de `OperationResult` (ni orquestador ni IPC), el cambio de discriminante no rompió llamadores; solo se ajustó `test/smoke.test.ts` (usaba `ok`).

**Parte C — MergeOrchestrator (decisiones documentadas en el header del archivo):**
  - **DECISIÓN 1** — dependencias inyectadas por constructor (los 7 componentes + `GamePaths` + FS propio + `workRoot`), mismo patrón del resto del dominio; testeable sin disco/procesos/vpk reales.
  - **DECISIÓN 2** — FS PROPIO `MergeOrchestratorFileSystem` (`ensureDir` + `copyFile` + `removeDir`), NO reutiliza el `BackupFileSystem` (contrato privado de BackupManager). El orquestador hace directamente 3 cosas de I/O: crear el workDir, instalar el `pak01_dir.vpk` en modsvs/ y limpiar el workDir.
  - **DECISIÓN 3 (mecanismo del workDir)** — `<workRoot>\merge-<epochMs>-<seq>` con contador incremental por instancia para unicidad; `workRoot` inyectado (bajo el prefijo `work/` ya ignorado por `.gitignore`); se limpia SIEMPRE en un `finally`.
  - **DECISIÓN 4 (inferida) — resolución add/remove:** `addAddon` hace UPSERT sobre `getManifest()` (si el addonId ya está, actualiza su `priorityOrder`; si no, lo agrega); `removeAddon` filtra ese addonId (no-op si ausente). Ambas materializan una fusión completa desde cero del Active_Set resultante (Req 8.3-8.5), no una operación incremental sobre el `.vpk`.
  - **DECISIÓN 5 (inferida) — addon candidato ausente del escaneo de la Workshop = fallo definitivo** con ese `addonId` (coherente con "fail-fast con estado consistente" de design.md; no hay AC explícito).
  - **DECISIÓN 6 — manejo REACTIVO uniforme** de las 3 escrituras en Game_Root (backup, instalar, gameinfo) vía `#writeStep`, que captura cualquier error y lo pasa a `handleWriteFailure`: `already-writable` -> propaga el error original como fallo definitivo (un `GameInfoEditError` de SearchPaths cae acá, sin elevar); `elevated-handoff` -> `status: "elevating"`; `denied` -> fallo por UAC cancelado. `MergeEngine.merge` NO va por `#writeStep` (opera en el workDir temporal fuera del Game_Root; sus `VpkToolError` se propagan directo con el addonId, Req 6.12).
  - **P-14 RESUELTO** — el `finally` de `#materialize` borra recursivamente el workDir en todos los casos (éxito, fallo, elevación), la limpieza que MergeEngine no hacía.
  - **Resume (18.2)** — `resumePendingOperation()` lee `getPendingSession()`; `null` -> devuelve `null` (nada que resumir); si hay sesión (incluso `[]`, ver `local-store.ts` DECISIÓN 5) materializa SALTEANDO la elevación proactiva (la instancia ya está elevada) y limpia con `clearPendingSession()` al terminar. El mecanismo de leer la PendingOperation de args/archivo al arrancar y el re-escaneo de la UI quedan FUERA de este método de dominio (son de la capa de composición, Tarea 20).

**Parte D — tests:** property test 18.3 (Property 13): compara los `orderedAddons` que el orquestador pasa a `MergeEngine.merge` entre `addAddon`/`removeAddon` y `applyActiveSet` con el Active_Set final resuelto (mismo conjunto y orden ascendente), con dobles en memoria y >=100 iteraciones. Unit tests 18.4 (20 casos): aborto por juego corriendo; orden `backup->merge->instalar->gameinfo->manifest` por log cronológico; aborto por backup no-permisos sin llamar a merge; `handleWriteFailure` disparado por EACCES/EPERM en cada uno de los 3 pasos con `elevating`/`denied`; `GameInfoEditError` no-permisos propagado sin elevar; corte proactivo por `elevated-handoff` antes de backup; upsert de `addAddon`/no-op de `removeAddon`; addon ausente del escaneo -> fallo con addonId; limpieza del workDir en éxito y fallo; `resumePendingOperation` (null / materializa sin ensureCanWrite y limpia / candidato vacío).

**Motivo (global):** cerrar la Sección 18 resolviendo los huecos de design.md (tercer estado del resultado, forma del orquestador, contrato de FS) con el mismo criterio de "documentar toda decisión de forma no trivial inferida" de las Secciones 10-17, y resolver los pendientes P-14 y P-15 que estaban explícitamente asignados a esta tarea.

**Alternativas descartadas:** reusar `ok: false` para el estado de elevación (descartada: mentiría, no falló; un booleano no da tres estados); reutilizar el `BackupFileSystem` para el FS del orquestador (descartada: acopla componentes por un contrato privado y no cubre `ensureDir`/`removeDir`); dejar `MergeEngine.merge` bajo el manejo reactivo (descartada: sus errores no son de permisos de Game_Root).

**Impacto:** `src/main/domain/merge-orchestrator.ts` (nuevo), `src/main/domain/types.ts` (`OperationResult` de 3 estados), `src/main/domain/elevation-service.ts` (`ensureCanWrite` + `operationType`), `src/main/domain/index.ts` (barrel), `test/merge-orchestrator.{test,property.test}.ts` y `test/helpers/orchestrator-doubles.ts` (nuevos), `test/elevation-service.test.ts` y `test/smoke.test.ts` (ajustes de firma/tipo), `Context/02-pendientes.md` (P-14 y P-15 marcados RESUELTO), `tasks.md` (checkboxes 18/18.1/18.2/18.3/18.4/19). Verificación: `npm run typecheck` limpio y `npm test` en 223/223 (30 archivos). Condiciona la Tarea 20 (capa IPC), que consumirá `MergeOrchestrator` y el nuevo `OperationResult`. Pendiente de merge a `main`.
...
### [2026-09-08] Sección 15 (LocalStore) — CORRECCIÓN: `getPendingSession()` distingue `null` de `[]` (DECISIÓN 5, amplía la Decisión 3)

**Qué:** Se cambió el comportamiento del estado de sesión pendiente en `src/main/domain/local-store.ts` respecto de lo entregado en la entrada anterior de la Sección 15. Antes, `getPendingSession()` devolvía `null` cuando `pending_session` no tenía filas, colapsando dos situaciones distintas ("no hay sesión" y "sesión guardada con candidato vacío") en `null`. Ahora las DISTINGUE:
  - `null` = no hay Active_Set candidato para rehidratar (nunca se guardó, o se limpió con `clearPendingSession`).
  - `[]` = hay una sesión ACTIVA cuyo Active_Set candidato es efectivamente vacío.
  - lista con entradas = el candidato tal cual se guardó.

**Motivo:** el caso `[]` es REAL, no teórico. `MergeOrchestrator.removeAddon` (tarea 18) sobre un Active_Set de tamaño 1 deja el candidato en vacío justo antes de un relanzo elevado; la instancia elevada DEBE rehidratar ese "vacío intencional" (aplicar un Active_Set vacío), no interpretarlo como "no había sesión que reanudar". La entrega anterior había anotado explícitamente que si la tarea 17 necesitaba esta distinción habría que revisitarlo — se revisitó aquí antes de que la tarea 17 la consuma.

**Cómo (DECISIÓN 5, documentada en el header del archivo):** se agregó una tabla de estado de UNA sola fila `pending_session_state (id INTEGER PRIMARY KEY CHECK (id = 1), active INTEGER NOT NULL DEFAULT 0)` que rastrea si hay sesión ACTIVA, independiente de cuántas filas tenga `pending_session`. `savePendingSession` marca `active = 1` SIEMPRE (incluso con lista vacía) y reemplaza las filas EN LA MISMA TRANSACCIÓN (filas + flag no pueden quedar desincronizados); `clearPendingSession` marca `active = 0` y vacía las filas, también en transacción; `getPendingSession` consulta el flag primero: si no está activo devuelve `null`, si lo está devuelve las filas tal cual (posiblemente vacías). Esto AMPLÍA la Decisión 3 original (que decía que "sin filas" colapsaba a `null`): esa parte de la Decisión 3 queda SUPERSEDIDA por la Decisión 5.

**Alternativas descartadas:** un flag `active` como columna dentro de la tabla `paths` (descartado: acoplaría el ciclo de vida de la sesión pendiente al de las rutas, que son estados ORTOGONALES —puede haber sesión pendiente sin rutas completas y viceversa— y obligaría a que la fila de rutas exista para marcar una sesión activa); seguir infiriendo de la cantidad de filas (descartado: es justo lo que no puede distinguir los dos casos).
...
### [2026-09-09] Rebuild nativo de better-sqlite3 y test de persistencia en archivo real (bloque 3 de la Tarea 20.4)
**Qué:** En la rama `ipc-preload`, resolución de los dos pendientes del "bloque 3" del composition root (Tarea 20.4). El código de dominio de persistencia (`SqliteLocalStore`, Sección 15) ya estaba cerrado y NO se tocó: recibe la `Database` de better-sqlite3 por inyección (DECISIÓN 2 de `local-store.ts`) y no maneja apertura/cierre del archivo. Lo pendiente era puramente de composición/empaquetado y de cobertura de persistencia real.
**DECISIÓN F — Script `rebuild:native`, NO en `postinstall`:** se agrega a `package.json` el script `"rebuild:native": "electron-rebuild -f -w better-sqlite3"` (sin tocar los cinco scripts existentes: `build`, `typecheck`, `test`, `test:watch`, `electron:pack`). Su función es recompilar el módulo nativo `better-sqlite3` contra la ABI del runtime de Electron, necesario para correr Electron REAL en desarrollo (no solo los tests). Se decide dejarlo como script MANUAL bajo demanda y NO como `postinstall` automático por dos motivos: (a) los tests corren bajo el Node del sistema vía Vitest, cuya ABI es compatible con un `npm install` normal, así que forzar un rebuild contra Electron en cada `install` rompería la suite de tests (quedaría compilado para la ABI de Electron, no la de Node); (b) `electron-builder` (`electron:pack`) ya dispara su propio rebuild nativo al empaquetar, así que el `postinstall` sería redundante para el flujo de release. `@electron/rebuild` ya estaba en devDependencies; solo faltaba el punto de entrada del script.
**DECISIÓN G — Test de integración con archivo real, separado de los `:memory:`:** se agrega `test/local-store-file.test.ts`, que abre una `Database` sobre un archivo `.sqlite` FÍSICO bajo `os.tmpdir()`, escribe paths/manifest/pending session, CIERRA la Database, la REABRE contra el mismo archivo con una instancia nueva de `SqliteLocalStore` y verifica que los datos sobrevivieron. Cubre lo que los tests contra `:memory:` (`local-store.test.ts`, `local-store.property.test.ts`) NO pueden probar por definición: la persistencia entre cierres. El directorio temporal se limpia en `afterEach` (recursive+force) incluso si el test falla. Dos casos: (1) round-trip paths/manifest/pending session tras cierre+reapertura; (2) reapertura sin escrituras previas deja `getPaths`=null, `getManifest`=[], `getPendingSession`=null (estado inicial correcto sobre archivo nuevo).
**Alternativas descartadas:** `postinstall` con rebuild automático (descartado por (a) y (b) de la DECISIÓN F); reescribir los tests de `:memory:` para usar archivo (descartado: `:memory:` es más rápido y suficiente para la lógica SQL; el archivo real solo aporta la dimensión de persistencia entre cierres, que se cubre en un test aparte y focalizado).
**Impacto:** `package.json` (script `rebuild:native`), `test/local-store-file.test.ts` (nuevo). NO se tocó `local-store.ts` (dominio cerrado), ni `main.ts`, ni el barrel (`SqliteLocalStore` ya se exporta como valor). Verificación: `npm run typecheck` limpio y `npm test` en verde. Pendiente el bloque 4 (ElevationOsProvider) y el bloque 5 (composition root en `main.ts`, que construirá la `Database` real en userData e inyectará el `SqliteLocalStore`).

---

### [2026-09-09] Cierre de la Seccion 20 (capa IPC/preload)
**Que:** Cierra la Seccion 20 completa: 20.1 (contrato de canales IPC + preload con contextBridge), 20.2 (handlers ipcMain.handle), 20.3 (tests de handlers con dominio mockeado) de sesiones previas, mas 20.4 en cinco bloques de esta sesion (adaptadores hoja de sistema, ManualPathProvider, SqliteLocalStore + rebuild nativo de better-sqlite3, ElevationOsProvider, composition-root + wiring final de main.ts). La app arranca de punta a punta: detecta y persiste rutas (con salida limpia via dialog.showErrorBox + app.quit() si Steam/L4D2 no se detectan, Decision Q), expone los ocho canales IPC, y resume una sesion de fusion pendiente tras un relanzo elevado por UAC bufferizando eventos de progreso hasta que el renderer los pide una sola vez (Decision D2a-i).
**Hallazgo corregido en el cierre:** las casillas 20/20.1/20.2/20.3/20.4 de tasks.md nunca se habian marcado pese a que el codigo de 20.1-20.3 ya estaba implementado y mergeado de sesiones anteriores - quedaron sin actualizar en tasks.md. Se marcaron las cinco en este commit de cierre, sin tocar codigo.
**Commits de esta sesion en ipc-preload (bloques de la 20.4):** dac60a6 (bloque 1, adaptadores hoja), 642c3cc (bloque 2, ManualPathProvider), 86342cd (bloque 3, rebuild nativo + test archivo real), d7690a2 (bloque 4, ElevationOsProvider), ba5775e (bloque 5a, composition-root testeable), 53dd043 (bloque 5b, wiring final de main.ts + fix del campo "main" de package.json).
**Impacto:** mergea a main. Typecheck limpio, build limpio, 311/311 tests. Proxima seccion: 21 (capa UI/renderer, todas sus subtareas marcadas opcionales en tasks.md) - queda para otra sesion.

---

### [2026-09-09] Regresion de la Seccion 20: el preload nunca cargo en runtime - se reemplaza "Vite solo para el renderer" por Vite en modo libreria tambien para el preload

**Que:** Durante la Tarea 21.1 (Bloque 1, Cover_Resolver) se detecto, verificando con Chrome DevTools Protocol (CDP) contra una ventana real de Electron 44.2.0, que `window.l4d2Api` es `undefined` en el renderer real pese a que la Seccion 20 (capa IPC/preload) se habia cerrado con 311/311 tests en verde. La consola real de DevTools mostraba: `Unable to load preload script: ...\dist\src\preload\preload.js` seguido de `SyntaxError: Cannot use import statement outside a module`. Es decir: NINGUN canal IPC funcionaba en runtime desde que se cerro la Seccion 20, y nadie lo habia notado porque los tests de esa seccion mockean el dominio y nunca cargan un preload real dentro de un Electron real (ver entrada `[2026-09-09] Cierre de la Seccion 20`).

**Causa raiz confirmada (no supuesta):** `tsconfig.json` compila TODO `src/**/*.ts` (incluido `src/preload/preload.ts`) con `module: "NodeNext"`, y `package.json` tiene `"type": "module"`, asi que el `preload.js` compilado por `tsc` queda en ESM real (`import { contextBridge, ipcRenderer } from "electron";` de nivel superior). `main.ts` NO fija `sandbox` en `webPreferences` (solo `contextIsolation: true, nodeIntegration: false`), y desde Electron 20 el sandbox esta activado por default para los renderers. Se confirmo contra la documentacion oficial de Electron (rama `main` de `electron/electron`, version instalada verificada con `npx electron --version` -> `44.2.0`) que un preload sandboxed se ejecuta como JavaScript plano SIN contexto ESM, sin importar la extension del archivo: `import` de nivel superior simplemente no puede funcionar ahi. La alternativa que si soporta ESM (preload sin sandbox + extension `.mjs`) implica deshabilitar el sandbox, una regresion de seguridad real frente al default vigente desde Electron 20, ademas de traer un caveat documentado de carrera en paginas con `<body>` inicialmente vacio.

**Decision (reemplaza la nota de `vite.config.ts`: "Vite se usa UNICAMENTE para el renderer; el proceso main y el preload siguen compilando con tsc"):** el preload se bundlea con Vite en MODO LIBRERIA (`vite.preload.config.ts` nuevo, `build.lib` con `formats: ["cjs"]`, `rolldownOptions.external: ["electron"]`) a un unico archivo CommonJS autocontenido, en vez de agregar un bundler nuevo al proyecto. Se eligio Vite (ya presente como devDependency para el renderer) porque al inspeccionar `node_modules/vite/package.json` se confirmo que la version instalada (8.2.2) trae **Rolldown** (`"rolldown": "~1.2.4"`) como bundler interno, no esbuild suelto - el modo libreria de Vite ya resuelve el grafo de dependencias del preload sin sumar herramientas. Esto NO fue una eleccion arbitraria de "agregar un bundler mas": fue la opcion recomendada explicitamente por la documentacion oficial de Electron para preload sandboxed con dependencias ESM, y la unica de las evaluadas que no reduce la postura de seguridad ya establecida en `main.ts` (sandbox activo, contextIsolation activo).

**Verificado con evidencia, no solo con tests:**
  - `dist/src/preload/preload.js` resultante: exactamente el path que `main.ts` ya esperaba (`path.join(__dirname, "../preload/preload.js")`), sin tocar ese wiring. Se forzo el `fileName` como funcion (`() => "preload.js"`) porque Vite, al ver `"type": "module"` en `package.json`, le habria puesto sufijo `.cjs` por default.
  - Contenido: `require("electron")` (externalizado, NO bundleado - lo resuelve el propio runtime de Electron) + el objeto plano `IPC_CHANNELS` de `src/main/app/ipc-contract.ts` (su unico import runtime real, el resto son `import type` que ya se borraban en compilacion) + la logica de `preload.ts`. Nada del dominio ni de `main.ts` se cuela en el bundle.
  - Verificacion end-to-end real via CDP contra una ventana de Electron real: `typeof window.l4d2Api` -> `"object"`, con los 9 metodos del contrato presentes, y `await window.l4d2Api.detectPaths()` devolviendo un `PathDetectionResult` real (`kind: "ready"` con las rutas detectadas), sin ningun error de carga de preload en consola.
  - `npm run typecheck` limpio (dos proyectos: main + renderer) y `npm test` en 331/331 (38 archivos).

**Integracion en los scripts (`package.json`):** `"build"` ahora encadena `tsc -p tsconfig.json && vite build --config vite.preload.config.ts` (el `tsc` sigue emitiendo un `preload.js` ESM inservible en el mismo path porque `src/preload` sigue incluido en `tsconfig.json` a proposito, para no perder el typecheck del preload; el build de Vite corre DESPUES en la misma cadena y lo sobrescribe con el CJS correcto - el orden es deliberado y no debe invertirse). Para `"dev"` se agrego `"dev:preload": "vite build --config vite.preload.config.ts --watch"` como tercer proceso de `concurrently`, y la pata que lanza Electron ahora espera con `wait-on tcp:5173 dist/src/preload/preload.js` (puerto Y archivo) antes de correr `tsc` + el build de Vite del preload una vez mas (para garantizar el archivo correcto en el primer arranque, sin depender del timing del watcher) y recien ahi lanzar `electron .`. Cambios posteriores a `preload.ts` se recompilan solos via el watcher, pero igual requieren reiniciar la ventana de Electron a mano para que el preload (que solo se inyecta una vez, al crear el `BrowserWindow`) se vuelva a cargar - esto es una limitacion inherente de Electron, no algo que el script pueda evitar.

**Motivo:** documentar que este cambio de herramienta de build NO es una preferencia de disenio nueva sino la correccion de un bug real de runtime que los tests de la Seccion 20 no podian detectar por construccion (mockean el dominio, nunca instancian un preload real). Deja registro de por que "Vite solo renderer" (decision implicita en el comentario de `vite.config.ts` al scaffoldear la Seccion 21) queda superada.

**Alternativas descartadas:** deshabilitar el sandbox + `.mjs` (regresion de seguridad real, con un caveat de carrera documentado que ademas habria que verificar caso por caso); un `tsconfig.preload.json` aparte con `module: CommonJS` sin bundler (funciona hoy porque el preload solo importa un objeto plano de otro archivo, pero es fragil: un import runtime nuevo en el futuro rompe en silencio si alguien olvida sumarlo al `include`, cosa que un bundler real no necesita que nadie recuerde); sumar esbuild como dependencia nueva (descartado porque Vite ya trae Rolldown y resuelve lo mismo sin agregar una herramienta).

**Impacto:** `vite.preload.config.ts` (nuevo), `package.json` (scripts `build`, `dev`, `dev:preload` nuevo), `dist/src/preload/preload.js` (ahora generado por Vite, no por `tsc`). NO se toco `src/preload/preload.ts`, `src/main/app/ipc-contract.ts`, `main.ts` (el path del preload que ya referenciaba sigue siendo valido), ni ningun test. Es un fix de build/runtime, NO de dominio. Se documenta separado del fix del protocolo de covers (`l4d2cover://`, Bloque 1 de la Tarea 21.1) porque son dos bugs independientes de secciones distintas descubiertos en la misma sesion de verificacion via CDP.

---

### [2026-09-10] COVER_EXTENSION duplicado entre AddonScanner y Cover_Resolver - extraido a modulo propio

**Que:** `/code-review` sobre el diff del Bloque 2 de la Tarea 21.1 encontro que `COVER_EXTENSION = ".jpg"` estaba definido DOS VECES: una copia privada en `addon-scanner.ts` (Seccion 6) y otra exportada en `cover-resolver.ts` (Seccion 21, Bloque 1). Riesgo real: si el formato de portada cambiara algun dia, actualizar solo una de las dos copias rompe la otra en silencio (el scanner seguiria reportando `coverPath != null` mientras el handler `l4d2cover://` buscaria la extension vieja, 404 en todas las portadas).

**Decision:** se extrajo la constante a un modulo nuevo y minimo, `src/main/domain/addon-cover.ts`, que NINGUNO de los dos modulos originales "posee" - `addon-scanner.ts` y `cover-resolver.ts` importan de ahi por igual. Se descarto que uno importara del otro (p. ej. `addon-scanner.ts` importando de `cover-resolver.ts`) porque invertiria la direccion natural de dependencia: `addon-scanner.ts` es una primitiva de dominio de la Seccion 6 (escaneo core), mientras que `cover-resolver.ts` es un modulo de soporte de UI de la Seccion 21 (protocolo `l4d2cover://`); que el primero dependiera del segundo leeria raro para cualquiera que navegue el codigo despues. El patron sigue el precedente ya establecido en el proyecto de modulos "core" compartidos entre dos consumidores relacionados (`collision-core.ts` entre `collision-resolver.ts` y sus consumidores).

**Refactor puro, sin cambio de comportamiento:** ningun test cambio sus aserciones; `npm run typecheck` y `npm test` (331/331) quedaron identicos a antes del refactor.

**Impacto:** `src/main/domain/addon-cover.ts` (nuevo), `addon-scanner.ts` y `cover-resolver.ts` (import en vez de constante propia), `index.ts` (el barrel ahora reexporta `COVER_EXTENSION` desde `addon-cover.js`). Commit separado del Bloque 2 de la Tarea 21.1 (UI), porque es un fix de un hallazgo de review sobre codigo de las Secciones 6/21-Bloque-1, no trabajo nuevo de UI.

---

### [2026-09-10] Adición de scope: preview de colisiones de solo lectura para la Sección 21.2 (`previewActiveSet`)

**Qué:** la Tarea 21.2 de `tasks.md` solo pedía "muestra el Active_Set instalado, permite agregar/quitar en caliente y reordenar el Priority_Order antes de fusionar; invoca los canales IPC correspondientes" — sin ninguna mención a un cálculo de colisiones ANTES de aplicar. Al leer el mockup de referencia (Claude Design, `Addon Manager.dc.html`, opción "2b - Compacto con flechas") elegido como fuente visual para esta tarea, el panel "Resumen de fusión" muestra colisiones/archivos a empaquetar antes del botón "Fusionar e instalar" — una capacidad que **no existía**: `MergeReport` (con las colisiones) solo se producía dentro de `applyActiveSet`, DESPUÉS de que el backup ya escribió en disco (`MergeOrchestrator#materialize`, Paso 4 backup antes que Paso 5 merge). No había ningún canal de preview sin efectos secundarios.

**Decisión:** se agrega `MergeEngine.preview()` y `MergeOrchestrator.previewActiveSet()` como una capacidad NUEVA de SOLO LECTURA (canal IPC `activeSet:preview`), reutilizando exclusivamente piezas ya existentes y probadas: `VpkTool.list()` (lectura pura del índice del VPK, ya usada por `AddonScanner`/`VScriptDetector`, sin extraer ni escribir nada) y `resolveMerge()` de `collision-core.ts` (núcleo puro de colisiones, Property 11 ya validada). NO se toca `MergeEngine.merge()`, `CollisionResolver.mergeInto` ni ninguna lógica de las Secciones 10-11 ya cerradas — `preview()` es un método nuevo y aislado que nunca invoca `MergeFileSystem` ni `CollisionResolver`.

**Tipo nuevo, NO se reutiliza `OperationResult`:** se define `ActiveSetPreview` (`types.ts`), una unión discriminada por `kind` (`"ready"` / `"addon-missing"`) deliberadamente MÁS SIMPLE que `OperationResult`: `previewActiveSet` nunca escribe, así que no existe un desenlace `"elevating"`, y el único caso "no exitoso" es una precondición de datos (un `addonId` ausente del escaneo de la Workshop), no el fallo de una operación en curso.

**Por qué NO hace falta el guard de escrituras (`operationInFlight`/`guardedWrite` de `ipc-handlers.ts`), ni `ProcessGuard` ni `ElevationService` — verificado LEYENDO el código, no supuesto:** se confirmó que NINGUNA operación de `applyActiveSet` escribe jamás en la Workshop_Folder — solo la LEE, exactamente lo mismo que hace `previewActiveSet`:
  - `AddonScanner.scan` (`addon-scanner.ts`) solo lee `workshopFolder` vía `FileSystem.listEntries`; cuando extrae `addoninfo.txt` (`#readAddonInfoSafely`), el destino es `this.#tempDir`, un directorio temporal PROPIO del scanner, nunca la Workshop_Folder.
  - `MergeEngine.merge()` extrae cada addon a `<workDir>\extract\<addonId>` (`#extractAddon`), donde `workDir` es el directorio temporal único por operación que crea `MergeOrchestrator#materialize` bajo `workRoot` — nunca dentro de la Workshop_Folder.
  - Las ÚNICAS escrituras reales de `applyActiveSet` en el Game_Root son backup/instalar (`paths.modsvsFolder`) y `gameinfo.txt` (`paths.gameInfoFile`) — rutas de `GamePaths` DISTINTAS y hermanas de `workshopFolder`, nunca la Workshop_Folder misma.

  Por lo tanto un `applyActiveSet` en curso y un `previewActiveSet` concurrente nunca compiten por una escritura: ambos solo LEEN la Workshop_Folder al mismo tiempo (lecturas concurrentes son seguras en cualquier FS), y `previewActiveSet` no toca `workDir` ni el Game_Root en absoluto. La evidencia completa (con líneas de código exactas) queda documentada como DECISIÓN 7 en `merge-orchestrator.ts` y `merge-engine.ts`.

**Fallos parciales — DIVERGE deliberadamente de `merge()`:** si `VpkTool.list()` falla para UN addon (VPK corrupto), `preview()` lo excluye BEST-EFFORT del cálculo (queda en `unavailable` con su motivo) y sigue calculando el resto — mismo criterio que `AddonScanner` usa para metadata. `merge()` real sigue abortando la fusión COMPLETA ante el mismo fallo (DECISIÓN 4 de `merge-engine.ts`, sin cambios). Ver P-19 en `02-pendientes.md`: la UI de 21.2 debe comunicar esta asimetría (un addon "unavailable" en el preview bloquea el "Fusionar e instalar" real, no se saltea solo).

**Property test nuevo — Property 16 (no estaba entre las 15 formalizadas originalmente en `design.md`):** "Preview y apply reportan las mismas colisiones para el mismo Active_Set". Agregada a `design.md` (sección Correctness Properties) y a `tasks.md`, y notas de Testing Strategy actualizadas de 15 a 16 properties. Requirió un `MergeEngine` REAL (no el `FakeMergeEngine` de `orchestrator-doubles.ts`, que devuelve un reporte fijo y volvería trivial la comparación) conectado a un `VpkTool` fake compartido entre ambos caminos, para que `previewActiveSet` y `applyActiveSet` atraviesen la MISMA lógica de colisiones (`resolveMerge`) a partir de los MISMOS datos.

**Numeración en `tasks.md` — corregida ANTES de commitear, tras revisión:** la primera versión de este cambio numeró la implementación como tareas nuevas 18.5/18.6/18.7 (bajo la Sección 18, ya cerrada y checkpointeada) y 20.5 (bajo la Sección 20, ídem), siguiendo el criterio "dónde vive el archivo tocado". Revisión posterior: las ramas `merge-orchestrator` y `ipc-layer` (Secciones 18 y 20) YA están mergeadas a `main`; el trabajo real de este cambio se está haciendo en la rama `ui` (Sección 21), así que el commit real cae ahí, no en esas ramas cerradas — numerar el checkbox bajo 18/20 separaba el checkbox del commit que realmente lo materializa. Se corrigió a subtareas **21.2a/21.2b/21.2c/21.2d** dentro de la entrada de la Tarea 21.2 (la sección/rama que efectivamente está hoy en curso), dejando las Secciones 18 y 20 intactas tal como quedaron en su checkpoint. Se agregó una regla explícita a `CONTRIBUTING.md` para que esto no vuelva a ser una decisión ad-hoc: una adición de scope que toca dominio de una sección ya cerrada/mergeada se documenta bajo la sección de la RAMA que está haciendo el trabajo ahora, no bajo la sección "dueña" del archivo tocado.

**Excluido deliberadamente de esta adición:** "Tamaño estimado" del panel del mockup NO se implementa — no hay dato de tamaño en ningún punto del dominio hoy (documentado como P-18 en `02-pendientes.md`).

**Motivo de encuadrarlo como ADICIÓN DE SCOPE y no como corrección:** 21.2 nunca prometió preview de colisiones; la especificación original no estaba mal — simplemente no contemplaba esta capacidad. Se agrega porque el mockup de referencia (elegido como fuente visual para esta tarea) no tiene sentido sin ese dato, y porque ya era construible con piezas existentes y probadas sin reabrir la lógica interna de Secciones 10-11.

**Impacto:** `src/main/domain/types.ts` (`ActiveSetPreview`, `MergePreview`, `UnavailablePreviewAddon`, nuevos), `src/main/domain/index.ts` (export de los tres tipos), `merge-engine.ts` (método `preview`, DECISIÓN 7), `merge-orchestrator.ts` (método `previewActiveSet`, DECISIÓN 7), `src/main/app/ipc-contract.ts` (canal `activeSet:preview` + `L4d2Api.previewActiveSet`), `src/main/app/ipc-handlers.ts` (handler nuevo, sin `guardedWrite`), `src/preload/preload.ts` (implementación del método nuevo de `L4d2Api`, necesaria para que el preload siga tipando contra el contrato — no estaba en el pedido original pero es mecánica, no UI), `test/merge-orchestrator-preview.test.ts` y `test/merge-orchestrator-preview.property.test.ts` (nuevos), `.kiro/specs/l4d2-versus-addon-manager/design.md` y `tasks.md` (Property 16). NO se tocó ningún componente React (esa es la Sección 21.2 propiamente dicha, todavía sin empezar).

---

### [2026-09-10] Fix del protocolo l4d2cover:// - host fijo "local" para servir portadas (Bloque 1, Tarea 21.1)

**Que:** el protocolo custom `l4d2cover://` (registrado como `standard:true` en `registerSchemesAsPrivileged`) que sirve las portadas de los addons al renderer no cargaba las portadas de ids NUMERICOS (el caso normal: los ids de la Workshop son numericos). La verificacion manual via CDP contra una ventana de Electron real mostro que la request de imagen salia con un host que era una direccion IP, no el id.

**Causa raiz - la heuristica host-como-IP de Chromium sobre esquemas `standard`:** con un esquema registrado como `standard:true`, Chromium parsea la URL con la maquinaria estandar de host/path (lo mismo que `http:`), y esa maquinaria aplica la canonicalizacion de host: un host puramente numerico se interpreta como direccion IPv4 y se reescribe. Ese `standard:true` NO es opcional: es lo que permite que un `<img src="l4d2cover://...">` cargue sin friccion desde el origen `http://localhost:5173` (dev). El problema es puramente COMO se ubica el id dentro de la URL.

**Los dos intentos que NO alcanzaron:**

  1. **Id en el host (`l4d2cover://<id>`, dos barras) - PRIMER bug.** El id numerico quedaba como host y Chromium lo canonicalizaba a IPv4 (p. ej. `3237709870` -> `192.251.136.46`), asi que el handler nunca recibia el id real. Este fue el bug original reportado.
  2. **Host vacio, id en el pathname (`l4d2cover:///<id>`, tres barras) - NO alcanzo.** La intuicion era que un host vacio dejaria el id como pathname y evitaria la heuristica. Pero para un esquema `standard`, Chromium NO tolera un host vacio: recupera el host consumiendo el PRIMER SEGMENTO del path como host. Al ser el id el primer segmento y ser numerico, cae exactamente en la misma canonicalizacion a IPv4. Verificado via CDP: `l4d2cover:///3237709870` -> `request.url` real `l4d2cover://192.251.136.46/`, pathname vacio, el handler devuelve 400 invalid-id.

**Fix - host FIJO literal "local" (`l4d2cover://local/<id>`).** Al fijar un host NO numerico y constante ("local"), el host ya esta ocupado por un valor que no dispara la heuristica de IP, y Chromium deja el id INTACTO como primer (y unico) segmento del pathname. El handler extrae el id del pathname (`new URL(request.url).pathname` sin la barra inicial), nunca del host. El host "local" es un valor centinela sin significado propio: solo existe para no dejar el host vacio.

**Evidencia (CDP contra ventana real):** con `l4d2cover://local/<id>` la request de portada resuelve 200 con el `<id>.jpg` correcto para ids numericos; los caminos de error controlados (id invalido -> 400, archivo ausente -> 404) siguen funcionando. Los 20 tests de `cover-resolver.test.ts` no cambiaron: operan sobre el id YA extraido, no sobre el parseo de la URL, asi que el bug era puramente de `main.ts` (como se arma/parsea la URL), no del dominio.
...
### [2026-09-10] Fix: la instancia sin privilegios NO se cerraba tras la elevación (reemplazo total)

**Qué (bug confirmado en QA):** al aceptar el prompt de UAC, la ventana SIN privilegios quedaba ABIERTA junto con la nueva instancia elevada -las dos coexistiendo-, contradiciendo directamente el diseño. design.md (ElevationService, Decisión 1 del ciclo de vida) y tasks.md 17.1 dicen textual: "la instancia sin privilegios se cierra una vez lanzada la elevada (reemplazo total, no coexisten)".

**Causa raíz - el cierre no existía en NINGUNA capa (no era un quit roto):** se trazó el camino completo del handoff y el cierre simplemente no estaba implementado. `RealElevationOsProvider.relaunchAsAdmin` lanza la instancia elevada y devuelve `"launched"` (correcto: es un adaptador de SO, no le toca cerrar la app). `ElevationServiceImpl.relaunchElevated` traduce a `elevated-handoff` (lógica pura, no conoce Electron). `MergeOrchestrator` corta el flujo y devuelve `status: "elevating"` (dominio, testeable sin Electron). Los handlers de `ipc-handlers.ts` retornaban ese `OperationResult` al renderer... y ahí moría la cadena: NADIE llamaba `app.quit()`. El único `app.quit()` existente era el de arranque fatal y el de `window-all-closed`. El diseño afirmaba "esta instancia se cierra sola", pero era una intención sin código que la respaldara.

**Fix - callback `onElevatedHandoff` inyectada desde main.ts (opción b):** se agregó `onElevatedHandoff: () => void` a `IpcHandlersDeps`. `ipc-handlers.ts` sigue SIN conocer `app` (importa electron solo como tipo); recibe la acción de cierre inyectada, igual que el resto de sus dependencias. En `main.ts` la callback es `() => app.quit()`. Se descartó ubicar el cierre en el dominio (MergeOrchestrator/ElevationService) para no acoplarlo a Electron y no perder su testabilidad.

**Por qué `app.quit()` y NO `app.exit()`:** `app.quit()` dispara el evento `before-quit`, donde vive el `db.close()` del better-sqlite3 (cierre limpio de la DB). `app.exit()` mata el proceso de inmediato, saltándose ese cierre. Como la instancia elevada tiene su PROPIA conexión a la misma DB en disco, dejar la vieja sin cerrar limpio es un riesgo innecesario.

**Cuidado de secuencia (explícito, no timing implícito):** el handoff se dispara DESPUÉS de tener listo el `OperationResult` con `status: "elevating"`, y se DIFIERE con `setImmediate` para que el handler RETORNE primero -el valor de retorno es lo que `ipcMain.handle` serializa y envía al renderer por IPC-. Si `app.quit()` arrancara antes del return, se arriesga a que el reply nunca viaje. `app.quit()` no mata el proceso sincrónicamente, pero igual se difiere para NO depender de ese timing implícito: el return sale en este tick, el quit arranca en el próximo. El disparo vive en `guardedWrite`, el envoltorio común de los tres handlers de escritura (apply/add/remove), único lugar donde puede aparecer `elevating`.
