...
- `App.tsx` consulta `getResumeState()` UNA vez cuando `isResuming()` resuelve `true` (ANTES de publicar `resuming`/`view`, para que `AddonList`/`ActiveSetPanel` ya monten con `pendingEntries` resuelto) y lo pasa hacia abajo. A diferencia del intento revertido, NO se limpia nunca despues: `pendingEntries` es un HECHO ESTATICO fijo para toda la vida del proceso (igual criterio que `isResuming`), y el usuario espera ver su seleccion restaurada de inmediato sin importar si la escritura de fondo ya fallo - el resultado se comunica aparte, por `OperationOverlay`.
