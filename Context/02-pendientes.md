...
### P-28 - BUG-007/BUG-013: "Activos" y los checkboxes de "Biblioteca" no restauran la seleccion pendiente tras el reinicio UAC - RESUELTO end-to-end

Reporte QA V3 (jornada 2). Tras un relanzo elevado, el Active_Set CANDIDATO completo (con `priorityOrder`) que el usuario tenia armado antes del handoff quedaba persistido del lado main, pero el renderer no tenia forma de leerlo: "Activos" (BUG-007, Critica) y los checkboxes "Incluir" de "Biblioteca" (BUG-013, Media) solo veian `getActiveSet()` (el manifest YA instalado), que durante un resume en curso -o si el resume termino en fallo- no refleja ese candidato.

**Intento inicial de esta sesion, REVERTIDO:** se habia implementado un canal IPC nuevo `activeSet:pendingSession` (`ipc-contract.ts`/`ipc-handlers.ts`/`preload.ts`) mas el consumo en `App.tsx`/`AddonList.tsx`/`ActiveSetPanel.tsx`. A mitad de sesion aparecio en el arbol de trabajo compartido el spec `.kiro/specs/bug-007-elevacion-pierde-batch/bugfix.md` de Kiro, que documenta una decision YA TOMADA por el usuario: **no** agregar un canal IPC nuevo tipo `getPendingOperation` si el flujo existente puede cubrir el caso extendiendo/renombrando lo que ya hay. Como el canal que se agrego contradecia esa decision y tocaba los mismos archivos que el spec de Kiro declara en su alcance, se revirtio por completo.

**RESUELTO (mitad main, Kiro, commit `141b353`):** `ResumeState` (`ipc-contract.ts`) gano el campo `pendingEntries: AddonManifestEntry[]` - EXTENDIENDO el tipo existente que ya expone `getResumeState()`, sin canal nuevo, tal como pedia la decision. Invariante: si el objeto `ResumeState` no es `null`, `pendingEntries` SIEMPRE esta presente (capturado en `composition-root.ts` desde el arranque, ANTES de que `resumePendingOperation()` limpie el pending), disponible tanto mientras el resume esta en curso como despues de terminar (exito o fallo).

**RESUELTO (mitad renderer, esta sesion), contra la forma real:**
- `App.tsx` consulta `getResumeState()` UNA vez cuando `isResuming()` resuelve `true` (ANTES de publicar `resuming`/`view`, para que `AddonList`/`ActiveSetPanel` ya monten con `pendingEntries` resuelto) y lo pasa hacia abajo.
- Logica de reconciliacion pura testeable (`src/renderer/state/pendingSelection.ts`, sin jsdom - ver P-28 nota anterior sobre infraestructura, sigue pendiente): `resolveActiveSetEntries` (el candidato pendiente tiene precedencia TOTAL sobre `getActiveSet()` cuando existe, usado por `ActiveSetPanel` para BUG-007) y `mergePendingIntoActive` (union por `addonId` con el candidato pendiente ganando en overlap, usado por `AddonList` para BUG-013).

**CORRECCION (hallazgo de revision QA sobre el commit `f4d539d`, esta sesion):** la version inicial dejaba `pendingEntries` fijo para siempre en `App.tsx` (nunca se limpiaba), asi que `resolveActiveSetEntries`/`mergePendingIntoActive` reaplicaban su precedencia total en CADA remontaje de `ActiveSetPanel`/`AddonList` (cambiar de pestaña y volver), no solo la primera vez tras el resume - un cambio real que el usuario hiciera despues del resume (agregar/quitar addons normalmente) se veria "revertido" visualmente en el proximo remontaje de cualquiera de los dos paneles. **Fix (commit `eb62e83`):** cada panel tiene su propio flag de consumo en `App.tsx` (`activeSetConsumedPending`/`addonListConsumedPending`), marcado por el panel mismo (`onPendingConsumed`) la primera vez que su propio `load()`/`runScan()` efectivamente usa `pendingEntries`; de ahi en adelante ese panel recibe `null` y cae al camino normal. Un flag COMPARTIDO no alcanzaba: el resume fuerza `view="active"` primero, asi que limpiar `pendingEntries` apenas lo consume `ActiveSetPanel` habria dejado a `AddonList` sin verlo NUNCA no-`null` (reabriria BUG-013 apenas el usuario entrara a "Biblioteca").
