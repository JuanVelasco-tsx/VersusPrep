...
### P-25 - BUG-004: backend listo (step "restarting" + isResuming), el renderer no consume ninguna de las dos senales (BLOQUEADO EN CODE)
...
**Accion pendiente (Code):** en el renderer, (1) agregar el caso del step `"restarting"` al overlay de progreso, y (2) consultar `isResuming()` al montar y adaptar el manejo de resume al modelo A1 (isResuming = "es un resume"; getResumeState().result = resultado terminal). Hasta ambos, BUG-004 NO esta cerrado end-to-end: el backend avisa, pero la UI todavia no reacciona.
